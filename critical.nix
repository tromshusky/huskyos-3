...
let
  ...
  updatescript = with pkgs; with config.huskyos; ''
    ...
    [ -d /boot/efi/boot ] &&
    echo ...building uki &&
    uki=$(nix build ${flakeFolder}#nixosConfigurations.huskyos.config.system.build.{uki,toplevel} --no-write-lock-file --print-out-paths --out-link /nix/var/nix/gcroots/next-system | head -n1)/nixos.efi &&
    echo ...installing uki &&
    cd /boot/efi/boot &&
